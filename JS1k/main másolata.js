f = r => {
    for (s = (a, b = a) => ({ x: a, y: b }), 
        ...
        if (!(A = h(p))[B=U = E = L = 0] || !(l = C(p, A[Math.random() * A.length | 0]))) {
            for (x = 0; x < u; x++)
                for (y = 0; y < v; y++) 
                    g(Z = s(x + 1, y + 1)) - 1 || (J = h(D = Z, 2)[0]) && (p = C(J, l = D), Q = x = y = u);
            Q--
        }
        S(o(C(p, p = l), 2), 2)
    }
...
(Y = _ => {
    ...
    k[++i] && L++;
    k[++i] && B--;
    g(n = C(I, o(t(L/20),396 / ((U = _) - U) / (B /= 1.6)))) && (I = n);
    I.x + I.y < 62 || E && E % 5 || f();
    ...
    for (x = 0; x < u; x++)
        for (y = 0; y < v; y++) 
            w = C(I, o(t(L/20 + (z = (x - u / 2) / u)), 150 / y * Math.cos(z))), g(w) && g(w) ^ 3 || (c.fillStyle = `rgb(${[0,0,0].fill(25 * (7+(w.x + w.y) % 3 | 0) + y / 30 | 0)}`, c.fillStyle = g(w) - 3 || "#0f0", c.fillRect(x, 200 - u / y / .01 / 2, 1, u/y/.01), y = v);
    u = v = 33;
    for (x = 0; x < u; x++)
        for (y = 0; y < v; y++) 
            c.fillStyle = ["#000", 0, V, "#0f0"][M[y][x]], c.fillRect(x * 3, y * 3, 3, 3);
    c.fillStyle = "#f00";
